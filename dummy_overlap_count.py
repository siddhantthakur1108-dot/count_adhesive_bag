...
track_count_label    = {}    # track_id → latest total count drawn above the bag dot
pending              = {}
graveyard            = {}
...
def commit_cross(track_id, direction):
    ...
    track_count_label[track_id] = count
...
while cap.isOpened():
    ...
    for track in tracks:
        ...
        if track_id not in track_confirmed_side:
            ...
            if old_id is not None:
                ...
                if old_id in track_count_label:
                    track_count_label[track_id] = track_count_label[old_id]
        ...
        # Show the actual running count just above the bag after a confirmed crossing.
        if track_id in track_count_label:
            count_txt = str(track_count_label[track_id])
            text_size, _ = cv2.getTextSize(count_txt, cv2.FONT_HERSHEY_SIMPLEX, 0.55, 2)
            text_x = max(2, min(FRAME_W - text_size[0] - 2, cx - text_size[0] // 2))
            text_y = max(18, cy - 14)
