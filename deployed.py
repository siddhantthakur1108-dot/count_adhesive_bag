...
from PyQt5.QtWidgets import (
    ...
    QLabel, QPushButton, QSizePolicy, QFileDialog, QMessageBox, QLineEdit
)
...
class CountingThread(QThread):
    ...
    MERGE_RADIUS_GREEN        = 15          # bag candidates
    MERGE_RADIUS_PINK         = 10
    COUNT_DEDUP_RADIUS	      = 4
    BAG_CONF                  = 0.2
    OVERLAP_CONF              = 0.60
    TRACK_IOU                 = 0.45
    ...
    def __init__(self, model_path: str, video_path: str, rtsp_url=""):
        super().__init__()
    ...
    def run(self):
        ...
        # Video source
        source = self._rtsp_url.strip()
        if source:
            video_path = source
        else:
            video_path = self._video_path

        is_rtsp = video_path.startswith(("rtsp://", "rtsps://"))
        ...
        if USE_HW_VIDEO_IO:
            if is_rtsp:
                gst_in = (
                    ...
                    'appsink sync=true max-buffers=1 drop=true'
                )
            else:
                gst_in = (
        ...
        def commit_cross(tid, direction, cx, cy):
            ...
            if counted_tracks.get(tid, False):
                print(
                    f"[BLOCKED] ID {tid} already counted "
                    f"(counted_tracks=True)"
             )
            return

            # Class-aware spatial de-dup (bag and overlap don't block each other)
            for rcx, rcy, rframe, r_is_ovr in recent_commits:
                if frame_number - rframe > 5:
                    continue

                distance = np.hypot(cx - rcx, cy - rcy)

                print(
                    f"[DEDUP CHECK] "
                    f"ID={tid}, "
                    f"distance={distance:.2f}, "
                    f"threshold={self.COUNT_DEDUP_RADIUS}, "
                    f"same_class={r_is_ovr == is_ovr}"
                	)
...
class FactoryDashboard(QMainWindow):
    ...
    def _build_picker_bar(self) -> QWidget:
        ...
        self._rtsp_edit.setPlaceholderText(("RTSP URL (leave empty use video)"))

    

        lay.addWidget(self._model_picker, 1)
        lay.addWidget(self._video_picker, 1)
        lay.addWidget(self._rtsp_edit, 2)
        return bar
    ...
    def _start_counting(self):
        ...
        rtsp_url = self._rtsp_edit.text().strip()
        self._thread = CountingThread(model_path, video_path,rtsp_url)
